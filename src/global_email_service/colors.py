...
DEFAULT_STATUS_COLOR_MAP: Dict[str, tuple[str, str]] = {
    "FAILED": _RED, "FAIL": _RED, "ERROR": _RED, "CRITICAL": _RED, "CBOS_ERROR": _RED,
    "SKIPPED": _YELLOW, "SKIP": _YELLOW, "TIMEOUT": _YELLOW, "WARNING": _YELLOW,
    "WARN": _YELLOW, "AGENT_RESTART": _YELLOW, "MANUAL_SKIP": _YELLOW,
    "IN_PROGRESS": _BLUE, "RUNNING": _BLUE, "PENDING": _BLUE, "INFO": _BLUE, "BLOCKED": _BLUE,
    "COMPLETED": _GREEN, "COMPLETE": _GREEN, "SUCCESS": _GREEN, "SUCCEEDED": _GREEN,
    "OK": _GREEN, "DONE": _GREEN,
}


def resolve_row_style(
    row: dict,
    color_overrides: Optional[Dict[str, tuple[str, str]]] = None,
) -> RowStyle:
    ...
    if explicit_color:
        label = str(
            row.get("severity") or row.get("alert_level") or row.get("segment_status")
