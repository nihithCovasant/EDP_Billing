...
row.current_phase via get_segment_state_handler(segment_code, phase), which
switches on segment family (real 7-step segments vs the 5 T+1 post-trade
processes, derived from segment_code) then phase within that family. If a
future migration ever adds a new SegmentPhase enum value without updating
both match/case branches, or a segment's phase and family get mismatched
some other way, the lookup raises ValueError. advance_pipeline() catches
that and marks the row FAILED — before this fix, an unmapped phase just
logged an error and returned the string "failed" WITHOUT ever touching
row.segment_status — the row stayed IN_PROGRESS at the same unmapped
phase, picked up again every cycle, hitting the same wall silently forever
(visible only in logs, never in the DB or any status API).

Reproduced here using a REAL SegmentPhase value that genuinely isn't wired
for the pipeline being driven: AWAIT_GTG only exists in the post-trade
branch, not the real-segment branch — so driving a real segment ("CUR")
through the pipeline while it's sitting at AWAIT_GTG is a faithful,
code-real instance of "no handler registered for this phase", no fake
enum value needed.
"""
