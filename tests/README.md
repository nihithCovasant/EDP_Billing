...
| `test_post_trade_processes.py` | **T+1 post-trade chain**: all 5 processes (`COLVAL, COLALLOC, MTFFT, DMRPT, DMSTMT`) complete successfully — run entirely independently of the 7 segments (none are even seeded); `COLALLOC` failing does not block the other 3 independent processes; Process 1's 02:30 IST window gate blocks/unblocks correctly. Also: a post-trade trigger resumed with an unconfirmed `"TRIGGERING"` marker (no CBOS-side status check exists for post-trade, unlike real segments) marks the process `FAILED` with an explicit "needs manual verification" reason and **never** calls the trigger endpoint again. |
