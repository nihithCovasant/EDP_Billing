...
generic WAITING_FOR_GTG -> [TRIGGERED ->] WAITING_FOR_COMPLETION pipeline.
...
async def test_post_trade_process_pending_past_deadline_fails_with_timeout(cfg, session_factory, test_date):
    """
    A post-trade process that never even started, past its window_end
    deadline, must FAIL/TIMEOUT rather than sit PENDING forever — mirrors
    _process_one_segment()'s equivalent check for real segments. Without
    this, a stuck CBOS dependency would silently never surface as an alert.
    """
    cbos = CbosClient(cfg.cbos_status_url, cfg.cbos_process_url, use_mock=True)
    orchestrator = EdpOrchestrator(cfg, cbos)
    ...
    assert "deadline" in (row.skip_reason or "").lower()


async def test_post_trade_process_in_progress_past_deadline_fails_with_timeout(cfg, session_factory, test_date):
    """
    A post-trade process CBOS never confirms (stuck polling
    WAITING_FOR_COMPLETION) must FAIL/TIMEOUT once its window_end deadline
    passes, via the same generic check AbstractStateMachine.execute_handler()
    already applies to real segments — proven here with a custom, earlier
    window_end so the test doesn't have to wait for the default 06:00.
    """
    cbos = CbosClient(cfg.cbos_status_url, cfg.cbos_process_url, use_mock=True)
    orchestrator = EdpOrchestrator(cfg, cbos)
    ...
    workflow_json = build_default_workflow_json(
        ...
        post_trade_processes=[
            {"process_code": "COLVAL", "login_id": "G_LID", "window_end": "03:30"},
        ],
    )
    async with session_factory() as session:
    ...
    fixed_now = helpers.fixed_post_trade_now_for(test_date, orchestrator._tz)  # 03:00 T+1
    async with session_factory() as session:
        row = await repository.get_one(session, test_date, "COLVAL")
...
async def _prime_triggering_post_trade_row(session_factory, test_date, fixed_now, *, code: str = "COLVAL") -> None:
    """Simulate a crash right after handle_triggered() committed the
...
async def test_post_trade_trigger_resume_fails_instead_of_retriggering(cfg, session_factory, test_date):
    """The "already triggered" check at WAITING_FOR_GTG only rules out a
    CBOS-confirmed prior run BEFORE entering TRIGGERED — once in TRIGGERED,
    an unconfirmed prior attempt (crash between committing "TRIGGERING" and
    recording the outcome) can never be safely auto-recovered. Resuming
